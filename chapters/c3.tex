...
本章考虑在EON中解决动态场景下的业务量工程问题，我们采用分层图模型将频谱分配问题简化为路由选择问题，设计出基于分层图模型的频谱分配与业务量工程算法（TESAA，traffic engineering and spectrum allocation algrithm），在进行频谱分配的同时优化路由代价。我们针对算法TESAA进行并行加速设计，在无权图和带权图两种情况下对TESAA进行GPU并行加速，实验发现，基于GPU的并行的PTESAA（parallel TESAA）算法对串行算法STESAA(serial TESAA)的加速比可达到5倍。我们将PTESAA和基于分层图模型的贪心算法比较，发现PTEAA在短时间内能够大大地优化路由代价，并且由于路由代价减小，占用网络资源较少，最终使得网络阻塞率降低。
\section{分层图模型}
